...
import com.igor.entities.Bullet;
import com.igor.entities.Enemy;
import com.igor.entities.Entity;
import com.igor.entities.Flor;
import com.igor.entities.Lifepack;
import com.igor.entities.Player;
import com.igor.entities.Weapon;
...
public class World {
			...
			for(int xx = 0; xx < map.getWidth(); xx++){
				for(int yy = 0; yy < map.getHeight(); yy++){
					...
					if(pixelsAtual == 0xFF000000) {
						...
						Game.enemies.add(en);
					}else if(pixelsAtual == 0xFFFF6A00) {
						//Weapon
						Game.entities.add(new Weapon(xx*16,yy*16,16,16,Entity.WEAPON_EN));
					}else if(pixelsAtual == 0xFFFF7F7F) {
						//Life pack
						...
						tiles[xx + (yy * WIDTH)] = new FloorTile(xx*16,yy*16,Entity.MOEDA);
					*/
				}
			}	
		} catch (IOException e) {
	...
	public static void restatGame(String level) {
		Game.entities.clear();
		Game.enemies.clear();
		Game.bullets.clear();
